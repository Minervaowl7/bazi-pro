          ...
          <div className="flex items-center gap-4">
            ...
            {pattern?.confidence !== undefined && (
              <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1 }}>
                <div role="progressbar" aria-valuenow={Math.round(pattern.confidence * 100)} aria-valuemin={0} aria-valuemax={100} aria-label="格局置信度" style={{ flex: 1, height: 6, background: "var(--surface-2)", overflow: "hidden" }}>
                  <div style={{
      ...
      <SchoolSectionCard title="子平用神（破格调整后）" icon="✦" color="var(--wx-water)">
        <div className="flex flex-col gap-3.5">
          <div className="flex items-center gap-3">
            <span className="font-bold text-[13px] px-3 py-1 rounded" style={{ background: "color-mix(in srgb, var(--success) 10%, transparent)", color: "var(--success)" }}>用神</span>
            <span className="text-lg font-bold" style={{ fontFamily: "var(--font-display)" }}><WuxingSpan text={yongshen?.yongshen || "—"} /></span>
          </div>
          <div className="flex items-center gap-3">
            <span className="font-bold text-[13px] px-3 py-1 rounded" style={{ background: "color-mix(in srgb, var(--wx-water) 8%, transparent)", color: "var(--wx-water)" }}>喜神</span>
            <span className="text-[17px]" style={{ color: "var(--text-2)" }}><WuxingSpan text={(yongshen?.xishen || []).join("、") || "—"} /></span>
          </div>
          <div className="flex items-center gap-3">
            <span className="font-bold text-[13px] px-3 py-1 rounded" style={{ background: "color-mix(in srgb, var(--danger) 8%, transparent)", color: "var(--danger)" }}>忌神</span>
            ...
            {breakConditions.map((bc, i) => (
              <div key={i} className="flex items-start gap-3">
                <span className="font-bold shrink-0 text-[13px] px-2.5 py-[3px] rounded" style={{
                  background: bc.severity === "high" ? "color-mix(in srgb, var(--danger) 8%, transparent)" : "color-mix(in srgb, var(--warning) 8%, transparent)",
          ...
          <div className="flex flex-col gap-2.5">
            {dayunVerdict.slice(0, 8).map((d, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <span className="font-bold shrink-0 text-[13px] px-2.5 py-[3px] rounded" style={{
                  background: d.verdict === "吉" ? "color-mix(in srgb, var(--success) 10%, transparent)" : d.verdict === "凶" ? "color-mix(in srgb, var(--danger) 8%, transparent)" : "color-mix(in srgb, var(--warning) 8%, transparent)",
