...
name: bazi-pro
description: >
  专业八字命理解读 Skill v3.3，配合 Bazi MCP 排盘数据使用。
  ...
  格局分析（月令透干定格+暗格+化气格+从格 全覆盖，含从强格/假从强格）、
  大运流年喜忌引动（含大运上限原则）、空亡/神煞/胎元命宫整合解读、
  六维度分述（性格/事业/财运/感情/健康/近运）+ 历史事件闭环校准。
  v3.3: 算析分离·线性执行流（废除暂定/回补）、两轮对话协议（详细版Turn1数据校准+Turn2全盘解读）、计算外包原则（LLM禁算数学）、燥湿寒暖偏枯预检、病药突破大运上限、动态负面清单比例。
...
**核心原则：排盘数据来自 MCP，你只负责解读。绝不自行推算天干地支，一切以 MCP 返回的数据为准。**
...
| P1-重要 | 十神、藏干、大运列表 | 大运缺失可跳过第六步；十神/藏干缺失可从天干地支自行推算 |
| P2-可选 | 纳音、神煞、命宫、胎元、空亡 | 标注"MCP 未返回"，跳过对应小节但不影响整体解读 |
| P3-补充 | 生肖、起运年龄、刑冲合会 | 标注"MCP 未返回"，从已有数据尽量推断 |
...
**你的数学计算能力不可靠。以下操作严禁在 Prompt 中手动执行：**
- ❌ 天干力量 = 0.5 × 0.7 × 0.7 等乘法链
- ❌ 地支藏干余气/中气的比例运算（×0.6、×0.3 等）
- ❌ 五行百分比手动加总

**你必须直接读取 MCP 工具返回的 JSON 数据**，仅作为「数据解说员」展示结果。若 MCP 未返回某项数值，标注「MCP 未提供」而非自行推算。

### 古籍检索标准化

**禁止生成** `python3 <skill_dir>/scripts/retrieve_classical.py` 命令。必须通过终端执行：
```bash
python3 /home/administrator/.hermes/skills/bazi-pro/scripts/retrieve_classical.py "<查询>" -k 5 --json
```
...
- **Step 4 喜用判定**：在格局和旺衰都已确定后，一次性输出，不再有「第四步完成后回到 §3.1 回补」的循环。
...
3. Step 5 五行力量分布（ASCII 柱状图 + 合化修正）
...
> **核心原则（出自《子平真诠》论用神）：格局决定质，扶抑调候决定量。** 调候是调节层，辅助参考，不可凌驾于格局用神之上。当调候与格局用神矛盾时，**一律以格局为准**。详见 3.5 冲突裁决规则。

#### 3.1 格局用神（主导层，由 Step 3 格局判定确定）
...
| 层级 | 内容 | 优先级 |
|------|------|--------|
| 第 0 层 | 格局用神（3.1） | **最高——决定命局成败方向，不可撼动** |
| 第 1 层 | 病药用神（3.4） | 与格局直接相关，解决格局内的结构缺陷 |
| 第 2 层 | 扶抑用神（3.2） | 兼顾日主强弱，修正格局偏向 |
| 第 3 层 | 调候用神（3.3） | 调节寒暖燥湿，优化格局长势 |
...
本气、中气均不透干 → 以月支本气十神定为暗格（如寅月不透甲→暗正财格/暗偏财格，取决于日主关系）。
...
> - 突破后的评分仍受原局日主有根约束：若日主无根（§4 层6 日主有根=0），突破上限减半（30-40→15-20）。
...
| **喜用神判定逆格局方向** | **格局用神永远第一优先。调候/扶抑与格局矛盾时，一律以格局为准（见 3.5 裁决规则）。例如庚金生于午月七杀格→格局用印（化杀），调候用水（壬癸）→泄印助杀，此时取印弃水** |
...
| **评分通胀（v3.2 新增）** | **格局评分必须参照 §4 层6 评分基准锚，不可所有命盘都给 50+ 分。用神被克→用神力度封顶 15/30；忌神当令无制→忌神受制封顶 0/20；日主无根→日主有根封顶 0/15。下等命盘（<40）禁止使用「潜力十足」「未来可期」等描述。** |
