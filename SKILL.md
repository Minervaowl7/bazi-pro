...
name: bazi-pro
description: >
  专业八字+紫微斗数联合分析 Skill v6.0 — 可审计、可交互、可视化的命理分析引擎（Replayable Evidence Engine），配合 Bazi MCP 排盘数据使用。
  ...
  v6.0: 八字+紫微联合分析——紫微斗数深度整合(大运/流年/四化API+联合面板+报告) + 流式Chat SSE + Function Calling工具集 + RAG向量融合检索。
  v5.3: 核心格局判定古籍对齐——会方检测收紧(对齐滴天髓) + 从强格官杀guard(对齐渊海子平) + 三刑优先两刑(对齐三命通会) + 阴干墓库加深(对齐滴天髓) + 调候整合喜神(对齐子平真诠) + Golden Cases 120个。
  v5.2: 盲派新派典籍对齐——盲派7项修正(体用定义/墓用复合/贼神捕神/五党成势) + 新派6项修正(反断同宗对/百神动态/身旺身弱综合/格局分类/出空机制)。
  ...
  v3.5: 层3暗格比劫拦截——月令本气为比肩/劫财时禁止生造"暗劫财格"等非经典术语，强制使用《子平真诠》"建禄月劫"框架（另寻财官煞食透干取用）。v3.4.1: 报告生成器跨平台字体修复（Linux/WSL 环境下 emoji→文本降级、中文字体栈修正 Noto Serif CJK SC、等宽字体 DejaVu Sans Mono、@font-face emoji 声明、PDF 渲染错误日志）。
    v3.3: 算析分离·线性执行流（废除暂定/回补）、两轮对话协议（详细版Turn1数据校准+Turn2全盘解读）、计算外包原则（LLM 只能解释 full_analysis 结果，复杂计算外包MCP）、燥湿寒暖偏枯预检、病药突破大运上限、动态负面清单比例。
...
**在执行检索之前，先从 `AnalysisEngine.analyze()` 返回结果的 `core_analysis.element_forces` 和 `core_analysis.wangshuai` 中读取五行力量分布和旺衰判定**，用于决定检索策略：
...
>
> ⚠️ **预检数据来源**：所有百分比和旺衰数据从 `core_analysis.element_forces` 和 `core_analysis.wangshuai` 读取，不由 LLM 粗估。
...
>
> 当命局处于以下极端寒暖燥湿状态时，调候可从调节层升格为**并立优先**：
>
> | 极端状态 | 条件 | 调候优先级 |
> |---------|------|-----------|
...
| 会方 | 五行 | 格局关联 | 解读 |
|------|------|---------|------|
| 寅卯辰 | 木 | 可成曲直格（专旺）[YHZ_00331] | 木气贯通，仁慈正直，文化教育方向 |
| 巳午未 | 火 | 可成炎上格（专旺） | 火势炎上，热情积极，能源传媒方向 |
| 申酉戌 | 金 | 可成从革格（专旺）[SFTK_00019] | 金气肃杀，刚毅果断，金融法律方向 |
| 亥子丑 | 水 | 可成润下格（专旺） | 水势润下，智慧流动，贸易物流方向 |
