...
name: bazi-pro
description: >
  专业八字命理解读 Skill v3.1.2，配合 Bazi MCP 排盘数据使用。
...
| 其他（<60% 且无两行≥85%） | 正常单通道检索（0.1） |

> **目的**：在解读者形成格局判断之前，先用数据驱动的方式预判命盘是否属于极偏/从格方向，避免"月令第一印象→格局锁定→检索被带偏"的确认偏误链。
>
> ⚠️ **预检方法的局限性**：上述公式仅统计天干透出和地支本气，**完全忽略藏干中气/余气、天干合化、地支合局对五行力量的修正**。因此上表增设 **60%-74% 灰区**——落在灰区时强烈建议升级为双通道检索，因为 Step 5 精确计算后合化等修正可能使实际占比跨过 75% 强制线。**第五步精确计算后若发现预检偏差 >15%，必须回溯重跑第〇步**（以精确计算重新确定通道策略），并在最终结论中注明预检偏差。
...
| 通道 A 得分 > 通道 B，但通道 B 命中从格经典条文（如《滴天髓》"假从""从象"） | ⚠️ **必须重新审视格局判定！** 不可因通道A分高就忽略通道B的从格证据 | 两通道条文同时引用，在第四步格局判定中展示从格 vs 正格的论据对比 |
| 通道 B 得分 > 通道 A，且命中假从/从格核心条文 | 🔴 **强烈信号：命盘极可能属于从格方向**。第四步层 0 优先检查从格，不可被羊刃/建禄抢先命中 | 优先引用通道 B 条文，通道 A 条文作为"为什么不是正格"的对比说明 |
| 两通道得分相近 | 格局存在歧义（可能为假从或正格均沾）。第四步展示两方面论据 | 均引用 |
...
- **第四步（格局判定）**：优先引用通道B的反事实结果（如果命中），作为格局判定的外部验证。引用「从象」「假从」「化象」「顺局」「格局成败」主题条文
- **第三步（喜用神判断）**：引用「用神取舍」「调候」「旺衰扶抑」主题条文
...
```
第一层（主导）：格局用神 → 由第四步格局判定确定月令十神格局之用神，决定命局成败方向
...
#### 3.1 格局用神（主导层，暂定——完整结论以第四步为准 ⚠️）

> ⚠️ **暂定结论**：第四步未执行时，以月令定格倾向暂代下方格局用神表。**第四步格局判定完成后，必须回到此处回补修正**（见末尾 §回补验证指令），并重新执行 3.5 四层综合裁决。完整结论以第四步为准。

格局用神是喜用神判断的**第一标准**，由 Step 4 的六层格局筛查确定：
...
> 若 Step 4 尚未执行（如精简版输出），以月令定格倾向暂代：月支藏干本气透出即以此为格局用神参考。完整结论以 Step 4 为准，届时可回补修正。
...
根据第四步的格局判定结果确定。**第四步未执行时，暂由月令初判暂代（如月令透正官则以正官为格局之病药框架），完整结论见第四步。⚠️ 第四步完成后必须回到此处回补修正！**
...
> ⚠️ **数据依赖说明**：以下格局的成格条件中，百分比阈值（如 ≥80%、≥85%）依赖第五步的五行力量精确计算。由于第五步在第四步之后执行，**层 0-A 暂用第〇步 0.0 五行快速预检的近似占比**进行初判。**第五步精确计算完成后，必须回到此处验证格局判定**：若精确计算后的占比与预检偏差 >10%，以精确计算为准修正格局结论（见第五步末尾 § 格局复核指令）。此机制与第三步的「暂定→回补」模式一致。
...
> ⚠️ **第四步格局判定完成后，必须立即回到以下章节回补修正，不得跳过：**

| 回到章节 | 回补内容 | 操作 |
|----------|---------|------|
| **§3.1 格局用神** | 用第四步的最终格局重新确定格局用神 | 覆盖月令暂代结论，填入实际格局对应的用神 |
| **§3.4 病药用神** | 用第四步的最终格局重新确定格局之"病"和"药" | 覆盖月令暂代结论 |
| **§3.5 裁决** | 重新执行四层综合裁决 | 以完整数据输出最终喜用神结论 |

> **回补完成后，在 §3.5 末尾标注「🔄 已回补：基于第四步 XX 格最终结论修正」以表明回补已执行。**
...
> ⚠️ **第五步精确计算完成后，必须回到第四步层 0-A 验证特殊格局判定是否仍然成立：**

| 检查项 | 操作 |
|--------|------|
| 精确占比与预检偏差 >10%？ | 以精确计算为准修正层 0-A 判定。标注「🔄 格局复核：精确计算 XX 行占比 XX%，预检为 XX%，偏差 X%，以精确计算为准」 |
| 层 0-A 此前命中某格局但精确计算后不再满足条件？ | 回退至该格局之前的判断位置，继续往下筛查。标注「🔄 格局复核：精确计算后 XX 格不再满足条件，回退重新筛查」 |
| 精确计算后发现此前层 0-A 遗漏的格局？ | 重新从层 0-A 顶部筛查，如有命中则覆盖此前判定。标注「🔄 格局复核：精确计算发现 XX 格条件满足，覆盖此前 YY 判定」 |

> **格局复核完成后，按第四步末尾的 §回补验证指令 继续执行 §3.1 / §3.4 / §3.5 的回补。**
...
引用 MCP 返回的大运列表（含起运年龄），结合第三步的喜用神判定结果：
...
| 40-59 | 中下 | 中等（接近60） | 下等 | 最好大运只能到中等边缘，不会出现跨越式发展 |
| <40 | 下等 | 中下等（接近40） | 下等 | 格局先天有重大缺陷，大运再好也不能翻盘为上等 |

> **禁用断语**：原局评分 <40 时，即使大运喜用神到位，禁止使用"格局发力""跨越式发展""一飞冲天"等描述。正确表述："运势有所改善，在原格局框架内向好发展。"
...
| 层级 | 触发条件 | 输出内容 |
|------|---------|---------|
| **精简版** | "我什么格局""喜用神是什么"等单点问题 | 第一步摘要 + 第四步格局结论 + 第三步喜用神结论，约 300-500 字 |
...
| **古籍有从格证据却置之不理** | **双通道检索中，通道B（反事实方向）只要命中 ≥1 条从格/假从/从象/顺局经典条文，必须在第四步格局判定中展示从格论据。不可因通道A分高就压制通道B的发现。** |
