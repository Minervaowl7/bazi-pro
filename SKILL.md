...
**核心原则：排盘数据来自 MCP，确定性计算由 bazi-pro 核心代码完成，你只负责解读。绝不自行推算天干地支、五行力量、十神、藏干、旺衰、格局、喜用神——这些全部由 `bazi_pro.core_rules.full_analysis()` 确定性计算得出，你只读取结果。禁止的操作：临场补算确定性代码已覆盖的内容、发明 MCP 未提供的数据、修改确定性计算的结果。**
...
**在执行检索之前，先从 `bazi_pro.core_rules.full_analysis()` 返回的 `element_forces` 和 `wangshuai` 结果中读取五行力量分布和旺衰判定**，用于决定检索策略：

```
预检数据来源（从确定性代码结果读取，不由 LLM 计算）：
- element_forces.percent：五行力量百分比
- wangshuai.verdict：旺衰判定
- pattern.candidates：格局候选列表
```
...
>
> ⚠️ **预检数据来源**：所有百分比和旺衰数据来自 `full_analysis()` 的确定性计算结果，不由 LLM 粗估。
