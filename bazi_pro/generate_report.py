...
def build_toc_html(sections: list[dict]) -> str:
    ...
    for idx, sec in enumerate(sections):
        ...
        next_level = sections[idx + 1]['level'] if idx + 1 < len(sections) else 0
...
def simple_md_to_html(text: str, anchor_map: dict = None) -> str:
    lines = text.split('\n')
    out = []
    in_table_head = False
    in_table_body = False
    ...
    while i < len(lines):
        ...
        if m:
            ...
            level = len(m.group(1))
            title = m.group(2).strip()
            if anchor_map and title in anchor_map:
                anchor = anchor_map[title]
            else:
                anchor = _title_to_anchor(title)
                if anchor in _anchor_counter:
                    _anchor_counter[anchor] += 1
                    anchor = f'{anchor}-{_anchor_counter[anchor]}'
                else:
                    _anchor_counter[anchor] = 0
            out.append(f'<h{level} id="{anchor}">{flush_inlines(title)}</h{level}>')
            i += 1
            continue
...
def markdown_to_html(text: str, anchor_map: dict = None) -> str:
    if anchor_map is not None:
        return simple_md_to_html(text, anchor_map)
...
def generate_html_report(meta: dict, body_text: str,
    ...
    anchor_map = {sec['title']: sec['anchor'] for sec in sections}
    body_html = markdown_to_html(body_text, anchor_map)
...
<div class="disclaimer">
  ...
  <br>
  <p>报告生成时间：{report_date} &nbsp;|&nbsp; bazi-pro v5.0 &nbsp;|&nbsp; 生成工具: scripts/generate_report.py</p>
</div>
...
def generate_enhanced_markdown(meta: dict, body_text: str,
    ...
    lines.append('---')
    lines.append('')
    lines.append(f'*报告生成时间：{report_date} | bazi-pro v5.0 | generate_report.py*')
    lines.append('')
    lines.append(
