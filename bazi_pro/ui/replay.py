...
def _default_stages(vm: DashboardVM) -> list:
    ...
    return [
        ...
        type('Stage', (), {'stage_id': '03', 'title': '格局筛查', 'summary': '六层筛查：特殊格局→建禄月劫→官杀混杂', 'confidence': 0.85, 'rules': ['从格三要件', '建禄月劫框架'], 'positive_evidence': [], 'counter_evidence': ['假从亦可发其身（《滴天髓》）']})(),
...
function selectStage(index) {
    ...
    const stages_data = [
        ...
        {title:'格局筛查',summary:'六层筛查：层0-A不通过（印比63%未达80%），层3建禄月劫·透官煞，层4官杀混杂→丁壬合去官留杀。',rules:'从格三要件·建禄月劫框架·六层筛查',pos:['层0-A全部不通过','建禄月劫透官煞成立','丁壬合去官留杀'],neg:['假从亦可发其身（《滴天髓》）']},
        {title:'双通道检索',summary:'通道A：建禄月劫·官杀方向（24.17分）；通道B：从格·假从方向（15.65分）。B不足以推翻A。',rules:'BM25+jieba·双通道裁决·反事实验证',pos:['A通道3条高分条文(24.17/22.61/21.36)','B通道仅1条从格条文(15.65)'],neg:['B通道命中假从条文——已纳入考量但不足以推翻']},
