...
def _cache_hmac_key() -> bytes:
    return hashlib.sha256(
        f"bazi-pro:bm25-cache:{_CACHE_VERSION}".encode()
    ).digest()
...
def get_bm25(corpus_path: str, entries: list[dict],
    ...
    if not force_rebuild and os.path.exists(cache_file):
        try:
            ...
            if not hmac.compare_digest(
                stored_sig, hmac.new(hmac_key, payload, hashlib.sha256).digest()
            ):
                raise ValueError("cache HMAC verification failed — file may be tampered")
    ...
    try:
        ...
        try:
            ...
            try:
                payload = pickle.dumps(cache_data, protocol=pickle.HIGHEST_PROTOCOL)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="八字古籍检索 (BM25 + jieba) v5.0")
