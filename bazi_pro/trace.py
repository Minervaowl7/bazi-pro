...
CORPUS_SIZE = None  # lazy load


def _resolve_corpus_path() -> Optional[Path]:
    candidates = [
        Path(__file__).resolve().parent / "data" / "classical_corpus.md",
        Path(__file__).resolve().parent.parent / "references" / "classical_corpus.md",
    ]
    ...
    if CORPUS_SIZE is None:
        ...
        if corpus:
            with corpus.open(encoding="utf-8") as f:
                CORPUS_SIZE = sum(1 for line in f if line.startswith("["))
        else:
            CORPUS_SIZE = 0
    return CORPUS_SIZE
...
def demo_trace() -> dict:
    ...
    tb.add_stage("retrieval_positive", title="顺向古籍检索", status="done",
                 ...
                 results=[
                     {"id": "SMTH_01164", "source": "三命通会", "score": 15.23,
                      "matched_terms": ["月劫", "比劫", "财官"]},
                     {"id": "ZPZ_00031", "source": "子平真诠", "score": 14.56,
                      "matched_terms": ["月劫格", "本身不可为用"]},
                 ])
    ...
    tb.add_stage("final_decision", title="最终裁决", status="done",
                 summary="月劫格，透官煞，印化通关。格局评分55/100中下等。用神火喜神木忌神水。",
                 confidence=0.79,
                 decision="月劫格，印化官煞，用火喜木忌水")
