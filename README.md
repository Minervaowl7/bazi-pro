...
**确定性命理计算核心 + 古籍对齐引擎 + 多流派分析路径 + 零幻觉叙述器 + 交互式 Web 应用**
...
三流派并行分析，统一 `SchoolAnalyzer` 基类接口：
...
源自民间盲派命理传统，以体用、宾主、做功为核心：
- 宾主分析：年月为宾（外环境），日时为主（内环境）
- 体用分析：体=比劫/印（我的工具），用=财/官/食（我的目标）
- 做功分析：制用（克制）、化用（化泄）、生用（相生）、合用（相合）
- 功力评估：高功/中功/低功/无功
- 应期预测：大运/流年引动时机
...
源自李涵辰新法，以用忌神为核心：
- 用忌神判定：月令定旺衰，身旺取克泄耗，身弱取印比
- 百神论：天干六亲替代法则（不现于天干时取月干/时干替代）
- 空亡论：六甲旬空亡查表，空亡地支力量减半
- 反断论：月令反断、空亡反断、旺极反断
...
| 盲派分析 | `bazi_pro.core.schools.mangpai` | 宾主/做功/体用/功力/应期 |
| 新派分析 | `bazi_pro.core.schools.xinpai` | 百神论/空亡论/反断论 |
| 103 Golden Cases | `tests/run_golden.py` | 103/103 通过 |
