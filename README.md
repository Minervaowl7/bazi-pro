...
**八字+紫微斗数联合分析引擎 · 确定性计算核心 · 古籍对齐引擎 · 多流派分析 · 流式 AI 对话 · Function Calling · 交互式 Web 应用**

2964 条古籍条文 · 6 部经典 · BM25+Hybrid 检索 · 六层格局筛查 · 破格检测 · 三流派分析（子平/盲派/新派）· 紫微斗数排盘+格局+四化 · 八字+紫微联合分析 · 流式 Chat SSE · Function Calling 工具集 · 确定性叙述器 · Next.js 暗色主题前端 · FastAPI SSE 后端 · 插件机制

**v6.0 亮点**：紫微斗数深度整合（大运/流年/四化飞星 API + 双轨时间轴 + 联合分析面板 + 紫微专属报告）· 流式 Chat SSE 对话（逐字显示 + reasoning 折叠 + 引用溯源）· Function Calling 工具集（排盘/格局/用神/神煞/古籍 5 个工具）· RAG 向量融合检索（BM25 + FAISS + 引用溯源）· 移动端适配（骨架屏 + 底部 Tab + 响应式布局）· 性能优化（11 个组件懒加载）· 首页引导优化（示例命盘 + 功能亮点 + SEO）
...
| 建禄/羊刃月令不入专旺格 | 子平真诠"用神专寻月令" | `not is_jianlu_yangren_month` |
...
| 建禄/羊刃月令不从强 | 子平真诠"用神专寻月令" | `not is_jianlu_yangren_month` |
...
- 大运时间轴（八字+紫微双轨并行，流年展开）
- 历史记录侧边栏
- **流派选择下拉框**（传统子平/盲派/新派/全流派对比）
- **全流派对比面板**（三列并排：子平/盲派/新派）
- **紫微斗数面板**（12 宫格 + 主星/辅星/煞星 + 四化飞星可视化）
- **八字+紫微联合分析面板**（维度对比 + 智能体洞察）
- **流式 Chat 面板**（SSE 逐字显示 + reasoning 折叠 + 古籍引用溯源卡片）
- **骨架屏加载**（5 个专属骨架屏组件）
- **移动端适配**（底部 Tab 导航 + 响应式布局 + 触摸友好）
- **错误状态 UI**（离线提示 + 重试按钮 + LLM 降级提示）
...
- `POST /api/v2/chat` — 命理问答（非流式）
- `POST /api/v2/chat/stream` — 命理问答（SSE 流式，支持 Function Calling）
- `POST /api/v2/report` — 详批报告生成（9 章节，含紫微斗数）
- `POST /api/v2/ziwei/chart` — 紫微斗数排盘
- `POST /api/v2/ziwei/horoscope` — 紫微斗数运势查询
- `POST /api/v2/ziwei/dayun` — 紫微斗数大运
- `POST /api/v2/ziwei/liunian` — 紫微斗数流年
- `POST /api/v2/ziwei/sihua` — 紫微斗数四化分析

**叙述器** (`bazi_pro/narrator.py`): 从计算结果直接生成命理师风格中文文本
- 9 个维度：总览/旺衰/格局/用神/调候/五行/刑冲/性格/事业
- 紫微斗数 6 个维度：命宫/格局/四化/财帛/官禄/综合
...
| 旺衰判定 | `bazi_pro.core.strength.judge_wangshuai` | 得令/得地/得势三维判定 + 官杀修正极旺 |
| 格局筛查 | `bazi_pro.core.patterns.screen_pattern` | 六层算法 (化气→专旺→从格→L1→L2→L3) + 建禄羊刃月令优先拦截 |
...
| 紫微斗数排盘 | `server/ziwei.py` | 基于 iztro-py，十二宫/主星/辅星/煞星/四化/五行局 |
| 紫微斗数格局 | `bazi_pro/core/ziwei/patterns.py` | 42 个格局检测（上格/中格/助力格/恶格） |
| 紫微斗数四化 | `bazi_pro/core/ziwei/sihua.py` | 本命/大限/流年三层四化分析 |
| 紫微斗数叙述 | `bazi_pro/core/ziwei/narrator.py` | 9 维度确定性叙述（命宫/格局/四化/财帛/官禄/夫妻/疾厄/综合/总览） |
| Function Calling | `server/agents/tools.py` | 5 个命理工具（排盘/格局/用神/神煞/古籍） |
| 流式 Chat | `server/routes/v2_chat.py` | SSE 逐字输出 + reasoning 折叠 + 工具调用 |
| 对话上下文 | `server/db.py` | SQLite 持久化 + 摘要压缩 + token 预算控制 |
| RAG 混合检索 | `server/rag_engine.py` | BM25 + FAISS 向量 + 引用溯源 + 结果缓存 |
| 507 Golden Cases | `tests/run_golden.py` | 120/120 通过（507 pytest tests） |
...
| `LLM_API_KEY` | (空) | LLM API Key（支持 OpenAI 兼容 API） |
| `LLM_API_BASE` | `https://api.openai.com/v1` | LLM API 基础地址 |
| `LLM_MODEL` | `gpt-4` | LLM 模型名称 |
| `BAZI_CHAT_CONTEXT_ROUNDS` | `10` | Chat 上下文轮数 |
| `BAZI_CHAT_TOKEN_BUDGET` | `4000` | Chat token 预算（超过时自动摘要压缩） |
...
| `bazi_pro/core/ziwei/` | 紫微斗数核心 | 42 个格局/三层四化/14 主星/9 维度叙述 |
| `bazi_pro/retrieve_classical.py` | BM25+古籍检索 | pickle 缓存 · `--batch` 批量 · JSON 元数据 |
| `bazi_pro/narrator.py` | 确定性叙述器 | 八字 9 维度 + 紫微 6 维度 · 零 LLM · 命理师风格 |
...
| `server/agents/` | 智能体编排 | 八字+紫微并行 → 综合解读 + Function Calling 工具集 |
| `server/llm.py` | LLM 客户端 | OpenAI 兼容 · 流式/非流式 · Function Calling · reasoning_content |
| `server/rag_engine.py` | RAG 引擎 | 问题分类 · BM25+Hybrid · 引用溯源 · 结果缓存 |
| `server/ziwei.py` | 紫微斗数服务 | iztro-py 封装 · 排盘/运势/大运/流年/四化/格局/综合分析 |
...
| 版本 | 内容 |
|------|------|
| **v6.0** | **八字+紫微联合分析平台**：紫微斗数深度整合（大运/流年/四化飞星 API + 双轨时间轴 + 联合分析面板 + 紫微专属报告章节 + PDF 导出）· 流式 Chat SSE 对话（逐字显示 + reasoning 折叠 + 古籍引用溯源卡片）· Function Calling 工具集（排盘/格局/用神/神煞/古籍 5 个工具 + LLM 自动调用）· RAG 向量融合检索（BM25 + FAISS + 引用溯源 + 结果缓存）· Agent 多轮对话上下文管理（SQLite 持久化 + 摘要压缩 + token 预算控制）· 移动端适配（骨架屏 + 底部 Tab + 响应式布局 + 触摸友好）· 错误状态 UI（离线提示 + 重试按钮 + LLM 降级提示）· 性能优化（11 个组件懒加载 + 代码分割）· 首页引导优化（示例命盘 + 功能亮点 + SEO） |
| **v5.3** | **古籍对齐修正版**：会方检测收紧（移除"任意三同五行≥3即会方"，严格要求寅卯辰/巳午未/申酉戌/亥子丑组合，对齐《滴天髓》"方是方兮局是局"）· 从强格增加官杀力量guard（官杀五行力量≥15%时不判从强，对齐《渊海子平》"官印双全"）· 三刑优先于两两相刑（对齐《三命通会》"三刑者三支相刑也"）· 阴干墓库扣分加深（乙丁己辛癸逢墓=-2，对齐《滴天髓》"阴逢库为无用"）· 调候用神自动整合到喜神（`derive_yongshen` 新增 `tiaohou` 参数，调候五行≠用神时加入喜神，对齐《子平真诠》"论用神配气候得失"）· 120 Golden Cases 全部通过 |
|------|------|
| **v5.2** | **典籍对齐版**：盲派 7 项方法论修正（体用定义修正/墓用复合做功/宾主细致化/贼神捕神检测/功神废神判定/五党成势/应期法则完善）· 新派 6 项方法论修正（反断论同宗对规则/百神论动态查找/身旺身弱综合判定/格局分类扶抑从强从弱/空亡出空机制/大运吉凶综合判定）· 对照典籍：段建业《盲派初级命理学》《命理珍宝瑰宝50期》、李涵辰《八字预测真踪》· Golden Cases 从 103 扩展至 507 · **v5.2 补丁**：建禄/羊刃月令优先于专旺格/从强格（对齐《子平真诠》"用神专寻月令"）· 旺衰极旺判定官杀修正（官印双全不强制极旺）· 神煞查表统一年支（对齐《三命通会》）· 天干合/地支冲映射表改用显式字典（替代 frozenset）· 紫微斗数排盘时辰修复 · 双源兼容取值（full_analysis/run_analysis）· 120 Golden Cases 审计全通过 |
