...
[![Version](https://img.shields.io/badge/version-3.1.2-brightgreen)](SKILL.md)
...
| **第三步：喜用神判断** | 四层架构（格局用神 → 病药验证 → 扶抑修正 → 调候调节）+ 量化裁决（0-100 分） |
| **第四步：格局判定** | 六层筛查：特殊格局 → 月令透干 → 暗格 → 混杂 → 成败 → 量化评分 |
...
```
bazi-pro/
├── SKILL.md                          # 主 Skill 定义（1177 行，AI Agent 执行规范）
├── scripts/
