...
def build_toc_html(sections: list[dict]) -> str:
    """生成带编号的目录 HTML（v4.3: 树形递归渲染，消灭 </ol></li> 重复闭合）"""
    if not sections:
        return ''

    # ── Step 1: 将平铺章节列表构建为嵌套树 ──
    root = {'level': 0, 'children': []}
    stack = [root]
    for sec in sections:
        while stack[-1]['level'] >= sec['level']:
            stack.pop()
        node = {'level': sec['level'], 'title': sec['title'],
                'anchor': sec['anchor'], 'children': []}
        stack[-1]['children'].append(node)
        stack.append(node)

    # ── Step 2: 递归渲染，保证 <li> 包裹完整子 <ol> ──
    def _render(children, prefix, is_root=False):
        if not children:
            return ''
        cls = ' class="toc-list"' if is_root else ''
        lines = [f'<ol{cls}>']
        for i, child in enumerate(children):
            num = f'{prefix}{i+1}' if prefix else str(i+1)
            sub_html = _render(child['children'], f'{num}.')
            lines.append(
                f'<li><a href="#{child["anchor"]}">'
                f'<span class="toc-num">{num}</span> '
                f'{escape(child["title"])}</a>'
            )
            if sub_html:
                lines.append(sub_html)
            lines.append('</li>')
        lines.append('</ol>')
        return '\n'.join(lines)

    inner = _render(root['children'], '', is_root=True)
