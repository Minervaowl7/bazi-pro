...
```js
import { getBaziDetail } from '/home/liyunlong/.npm/_npx/6c6c93aa84f1c2cf/node_modules/bazi-mcp/dist/index.js';
...
```bash
cd /home/liyunlong/.npm/_npx/6c6c93aa84f1c2cf/node_modules/bazi-mcp && node /tmp/bazi-query.mjs
```
...
找到后，将脚本中的 `import` 路径和 `cd` 目录替换为实际路径。常见替代位置（按优先级）：\n- `~/.npm/_npx/<hash>/node_modules/bazi-mcp/dist/index.js`（npx 缓存，**最常见**）\n- `~/node_modules/bazi-mcp/dist/index.js`（全局安装）\n\n> **当前用户环境已知路径**：`/home/liyunlong/.npm/_npx/6c6c93aa84f1c2cf/node_modules/bazi-mcp/dist/index.js`\n> （npx 缓存 hash 可能随包更新变化，若不可用请重新 `find`）
