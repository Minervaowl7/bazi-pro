...
| `core_rules.py` | 向后兼容代理，re-export 全部符号 from `bazi_pro.core`；触发 DeprecationWarning | — |

### 3.1.1 确定性命理计算核心 (`bazi_pro/core/`)

`bazi_pro/core/` 子包包含所有确定性的命理计算逻辑（十神、藏干、五行力量、旺衰、格局候选、喜用神候选、刑冲合害）。LLM 只解读结果，不执行计算。

| 文件 | 职责 |
|------|------|
| `constants.py` | 权威常量：GAN_WUXING, ZHI_WUXING, GAN_SHISHEN_MAP, derive_shishen |
| `stems.py` | 天干五合、五行相生相克映射 |
| `branches.py` | 地支藏干、十二长生、刑冲合害关系表 |
| `hidden_stems.py` | 藏干展开 (get_canggan) |
| `ten_gods.py` | 十神关系、旺衰方向辅助函数 |
| `relations.py` | 刑冲合害检测 (detect_relations) |
| `elements.py` | 五行力量计算 (calc_element_forces) — 含藏干权重、月令权重 |
| `strength.py` | 得令/得地/得势 + 旺衰综合判定 |
| `patterns.py` | 六层格局筛查 (screen_pattern) |
| `yongshen.py` | 四层喜用神推导 (derive_yongshen) |
| `__init__.py` | full_analysis() 主入口 + 全部 re-export |

**双层架构说明**：`bazi_pro/core_rules.py` 是向后兼容代理层，它 `from bazi_pro.core import *` 并触发 `DeprecationWarning`。外部代码可以通过 `from bazi_pro.core_rules import full_analysis` 或 `from bazi_pro.core import full_analysis` 两种方式访问——推荐后者。`AnalysisEngine.analyze()` 内部使用 `from bazi_pro.core_rules import full_analysis` 以保证向后兼容。
...
*本文档由项目代码分析自动生成，涵盖 bazi-pro v5.0.0 的完整架构、模块、类函数、依赖与运行方式。*
