...
@pytest.mark.asyncio
async def test_get_db_initializes_tables():
...
@pytest.mark.asyncio
async def test_get_db_returns_singleton():
...
@pytest.mark.asyncio
async def test_get_db_reconnects_after_close():
...
@pytest.mark.asyncio
async def test_list_analyses_pagination():
    ...
    assert len(page3["analyses"]) == 1  # 最后一页只有 1 条
...
@pytest.mark.asyncio
async def test_insert_and_get_chat_messages():
...
@pytest.mark.asyncio
async def test_get_chat_messages_school_filter():
    ...
    # 不传 school 应返回所有（不含 summary）
    all_msgs = await db_mod.get_chat_messages(aid)
    assert len(all_msgs) == 3


@pytest.mark.asyncio
async def test_get_chat_messages_excludes_summary():
...
@pytest.mark.asyncio
async def test_insert_and_get_latest_summary():
...
@pytest.mark.asyncio
async def test_get_latest_summary_school_filter():
...
@pytest.mark.asyncio
async def test_get_messages_after_id_limit():
...
@pytest.mark.asyncio
async def test_save_report_without_citations():
...
@pytest.mark.asyncio
async def test_get_report_returns_latest():
...
def test_generate_analysis_id_format():
    ...
    assert len(aid) == 16  # "ana_" (4) + 12 hex chars
